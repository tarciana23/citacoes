const quotes = [
  ...
  const quotesBnt = document.querySelector("#quoteBtn");
  const quoteText = document.querySelector(".text");
  const quoteAuthor = document.querySelector(".autor");

  function getQuote(){
    const index = Math.floor(Math.random() * quotes.length);
    // o math.random gera um número aleatório de 0 até a quantidade limite do array
    // o math.floor arredonda a qt para um número inteiro
    //depois pega esse número e passa como index para buscar a qupte e o autor
  ...
  quotesBnt.addEventListener("click",getQuote)